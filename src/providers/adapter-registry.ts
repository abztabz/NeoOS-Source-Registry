...
export function createDefaultAdapters(env: Record<string,string|undefined> = process.env): Record<string,GatewayAdapter> {
  const adapters:Record<string,GatewayAdapter>={"gdelt-doc":adaptSourceProvider(gdeltAdapter()),crossref:adaptSourceProvider(crossrefAdapter()),datacite:adaptSourceProvider(dataciteAdapter()),"world-bank-indicators":adaptSourceProvider(worldBankAdapter())};
